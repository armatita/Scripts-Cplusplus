...
{
	cout << "******************************************************\n";
	cout << "Starting remove row if no-data algorithm.\n";
	cout << "******************************************************\n";

	char conf[256]="removenodatarow.pin";
	...
	cout << "Finished.\n";
	cout << "Removing if no data and writing output file into disk. ";
	outputfile=fopen(filename2,"w");
	fprintf(outputfile,"Remove_nodata_at_row_%d\n",column);
	fprintf(outputfile,"%d\n",number_of_columns);
	for (i=0; i < number_of_columns; i++){fprintf(outputfile,"variable%d\n",i+1);}

	int j;
	for (i=0; i < number_of_rows; i++){
		if (data[i][column-1]!=nodata){

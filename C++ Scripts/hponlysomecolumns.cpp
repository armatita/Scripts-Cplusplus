...
{
	cout << "******************************************************\n";
	cout << "Starting only some columns algorithm.\n";
	cout << "******************************************************\n";

	char conf[256]="onlysomecolumns.pin";
	...
	fscanf(fileconf,"%d\n",&nums);
	int i;
	for (i=0;i<nums;i++){fscanf(fileconf,"%d\n",&cols[i]);}
	...
	for (i=0; i < 9; i++){
		if (cols[i]>number_of_rows){
			cout << "ERROR: The column number(s) is superior to the actual number of the columns in file. Check the parameters." << endl;
			return 0;
		}
	}
	cout << "******************************************************\n";
	...
	cout << "Finished.\n";
	cout << "Writing output file into disk. ";
	outputfile=fopen(filename2,"w");
	fprintf(outputfile,"Selected_columns\n");
	...
	for (i=0; i < number_of_rows; i++){
		for (j=0; j < nums; j++){
			if (j==nums-1){fprintf(outputfile,"%f\n",data[i][cols[j]-1]);}
			else {fprintf(outputfile,"%f\t",data[i][cols[j]-1]);}
		}
	}
	cout << "Finished.\n";
	return 0;

...
int sort_bi_histogram(float **bi_histogram,int size){
	int i;
	int j; // Just another loop counter...
	float value; // We need this for saving the sorted value while being transfered in the array.
	for (i=0; i < size; i++) {
		for (j=i; j < size; j++) {
			value = bi_histogram[j][0];
			if (value < bi_histogram[i][0]) {
				bi_histogram[j][0]=bi_histogram[i][0];
				bi_histogram[i][0]=value;
				value = bi_histogram[j][1];
				bi_histogram[j][1]=bi_histogram[i][1];
				bi_histogram[i][1]=value;
			}
		}
	}
	return 0;
...
{
	cout << "******************************************************\n";
	cout << "Starting sort bi-distributtion algorithm.\n";
	cout << "******************************************************\n";

	char conf[256]="sortbihist.pin";
	...
	cout << "Finished.\n";
	cout << "Sorting columns with reference to first... ";
	sort_bi_histogram(data,rows);
	cout << "Finished.\n";

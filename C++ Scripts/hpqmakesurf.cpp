...
int organize_my_data(float *surface,float **data, int cz,int bx,int by,int rows,float nodata,char type[4]){
	int i;
	int j;
	int counter=0;
	if (strcmp("X2Y",type)==0){
		for (i=0 ; i < bx*by ; i++){
			if (i<rows) {surface[i]=data[i][cz-1];}
			else {surface[i]=nodata;}
		}
	}
	else {
		for (i=0 ; i < bx ; i++){
			for (j=0 ; j < by ; j++){
				if (j*bx+i < rows){
					surface[counter] = data[j*bx+i][cz-1];
					counter = counter + 1;
				}
				else {
					surface[counter] = nodata;
					counter = counter + 1;
				}
			}
		}
...
int main()
{
	...
	cout << "Number of blocks in X is: " << bx << endl;
	cout << "Number of blocks in Y is: " << by << endl;
	cout << "Depth column is: " << cz << endl;
	...
	if (rows!=(bx*by)){cout << "Warning: grid size (" << bx*by << ") is different from horizon size (" << rows << ")." << endl;}
	cout << "****************Grid specifications*******************\n";
	cout << "Surface grid has " << bx << " number of blocks in X.\n";
	cout << "Surface grid has " << by << " number of blocks in Y.\n";
	if (strcmp("X2Y",type)==0){cout << "By your choice you've chosen input file order first by rows of X then of Y.\n";}
	else {cout << "By your choice you've chosen input file order first by rows of Y then of X.\n";}
	cout << "Allocating surface grid in memory... ";
	float *surface;
	surface = new (nothrow) float[bx*by];
	cout << "Finished.\n";
	cout << "Surface grid has 1 column and " << bx*by << " rows.\n";
	cout << "**************Building surface data*******************\n";
	cout << "Organizing result grid... " << endl;
	organize_my_data(surface,data,cz,bx,by,rows,nodata,type);
	cout << "Finished.\n";
	cout << "Writing to file...";
	simple_write_multi_array_to_file(filename2,data,rows,columns);
	//surface_write_single_array_to_file(filename2,surface,bx*by,bx,by,0,0,nodata,nodata,nodata);
	cout << "Finished.\n";
	return 0;
}
